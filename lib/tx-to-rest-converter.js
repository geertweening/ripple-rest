 ...
 */
TxToRestConverter.prototype.parsePaymentsFromPathFind = function(pathfindResults, options) {
  if (typeof options === 'function') {
    callback = options;
    options = {};
  }
  if (options && options.destination_amount) {
    pathfindResults.destination_amount = options.destination_amount;
  }
  if (options && options.source_account) {
    pathfindResults.source_account = options.source_account;
  }
  var payments = [];
...
TxToRestConverter.prototype.parseSettingResponseFromTx = function(message, meta, callback) {
  var result = {
    settings: {}
  };
